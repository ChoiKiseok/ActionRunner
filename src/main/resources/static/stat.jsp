    ...
    <script type="text/javascript" language="JavaScript">
      ...
      function getSubList(vwcd, listLev, parentId) {
        ...
        var paraObj = {
          ...
          url:
            "http://[개발자 홈페이지 주소]/devGuidePop.jsp?method=getList&key=ZjZjOTI3MjRjNmU1YzdhZTMwOWRjNjgxN2MzNDgwNmY=&vwcd=MT_ZTITLE&parentId=" +
            parentId +
            "&type=json",
          sync: true,
...
<c:set var="key"
  ...
  (request.getParameter("key"))); %></c:set
>
<c:set var="vwcd"
  ><%out.println(request.getParameter("vwcd") == null ? "" :
  (request.getParameter("vwcd"))); %></c:set
>
<c:set var="parentId"
...
<c:set var="type"
  ...
  (request.getParameter("type"))); %></c:set
>

<c:import
  url="http://kosis.kr/openapi/Expt/statisticsList.do?method=${method}&apiKey=${key}&vwCd=${vwcd}&parentListId=${parentId}&format=${type}"
  charEncoding="utf-8"
/>
